#!/usr/bin/env python3
"""
Generate Circle STARK verifier.simf (constant-based, optimized).

Precomputed twiddle inverses, no m31_inv.
Supports toy/micro/medium/large/production configs.
"""
...
def generate_proof(log_trace_size, log_blowup, n_queries, n_fri_layers, seed=42):
    ...
    print(f"  {domain_size} domain points")

    trace_evals = [random.randint(0, P-1) for _ in range(domain_size)]
    trace_tree = MerkleTree([hash_m31_leaf([v]) for v in trace_evals])
    ...
    for q_idx in query_indices:
        ...
        for li in range(n_fri_layers):
            ...
            q['twiddle_invs'].append(m31_inv(m31_mul(2, t)))
            ci = fi
        queries.append(q)
...
def generate_verifier(proof):
    ...
    for qi in range(nq):
        lines.append(f"    let (state, _qi): (u256, u32) = channel_squeeze_index(state, {mask_str});")
    lines.append("")
